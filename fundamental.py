...
nation = st.selectbox(
    ...
    options=["None","Saudi Arabia", "United States"],
    index=None,
    help="Choose the stock market first. You can change this selection at any time.",
)
if nation == "None":
    st.stop()
...
if nation == "Saudi Arabia":
    ...
    ticker_symbol = st.text_input("Enter Saudi Stock ticker")
    ...
    ticker_symbol = st.text_input("Enter US Stock ticker")
...
sharesP1 = get_df_value(balance, ["Share Issued", "Ordinary Shares Number", "Shares Outstanding"], 1, 0)
...
if statement_type == "(A) yearly":
    ...
    sharesP1 = get_df_value(balance, ["Share Issued", "Ordinary Shares Number", "Shares Outstanding"], 1, 0)
    epsP1 = get_df_value(financial, ["Basic EPS", "Diluted EPS", "EPS"], 1, 0)
    navP1 = (assetP1-liabilitiesP1)/sharesP1
    roep1 = incomeP1/equityP1
    roaP1 = incomeP1/assetP1
    revenue_growthP1 = (revenueP1 / get_df_value(financial, ["Total Revenue", "Revenue"], 2, revenueP1)) - 1
    earning_growthP1 = (incomeP1 / get_df_value(financial, ["Net Income", "Net Income (Loss)"], 2, incomeP1)) - 1
    eps_growthP1 = (epsP1 / get_df_value(financial, ["Basic EPS", "Diluted EPS", "EPS"], 2, epsP1)) - 1
    freecashflow_growthP1 = (get_df_value(cashflow, ["Free Cash Flow"], 1, 0) / get_df_value(cashflow, ["Free Cash Flow"], 2, get_df_value(cashflow, ["Free Cash Flow"], 1, 0))) - 1
    ...
    price_P1 = round(data["Close"].iloc[year_ago],2)
    freecashflow_yieldP1 = get_df_value(cashflow, ["Free Cash Flow"], 1, 0)/(price_P1*sharesP1) if sharesP1 else 0
    price_to_bookP1 = price_P1/(equityP1/sharesP1) if sharesP1 else 0
    ev_ebitda_p1 = ((price_P1*sharesP1)+get_df_value(balance, ["Total Debt"], 1, 0)-get_df_value(balance, ["Cash And Cash Equivalents", "Cash and Cash Equivalents"], 1, 0))/ebitdaP1 if ebitdaP1 else 0
    ffo_pershareP1 = get_df_value(cashflow, ["Funds From Operations"], 1, 0) / sharesP1 if sharesP1 else 0
    price_to_ffoP1 = price_P1 /ffo_pershareP1 if ffo_pershareP1 else 0 
    ...
    total_returnP1 = ((dividendsP1/sharesP1)+(repurchase_p1/sharesP1))/price_P1 if price_P1 else 0
    name1 = "current year"
    name2 = "past year"
    ...
    sharesP1 = get_df_value(balance, ["Share Issued", "Ordinary Shares Number", "Shares Outstanding"], 4, 0)
    epsP1 = get_df_value(financial, ["Basic EPS", "Diluted EPS", "EPS"], 4, 0)
    navP1 = (assetP1-liabilitiesP1)/sharesP1
    roep1 = incomeP1/equityP1
    roaP1 = incomeP1/assetP1
    revenue_growthP1 = (revenueP1 / get_df_value(financial, ["Total Revenue", "Revenue"], 8, revenueP1)) - 1
    earning_growthP1 = (incomeP1 / get_df_value(financial, ["Net Income", "Net Income (Loss)"], 8, incomeP1)) - 1
    eps_growthP1 = (epsP1 / get_df_value(financial, ["Basic EPS", "Diluted EPS", "EPS"], 8, epsP1)) - 1
    freecashflow_growthP1 = (get_df_value(cashflow, ["Free Cash Flow"], 4, 0) / get_df_value(cashflow, ["Free Cash Flow"], 8, get_df_value(cashflow, ["Free Cash Flow"], 4, 0))) - 1
    ...
    freecashflow_yieldP1 = get_df_value(cashflow, ["Free Cash Flow"], 4, 0)/(price_P1*sharesP1) if sharesP1 else 0
    price_to_bookP1 = price_P1/(equityP1/sharesP1) if sharesP1 else 0
    ev_ebitda_p1 = ((price_P1*sharesP1)+get_df_value(balance, ["Total Debt"], 4, 0)-get_df_value(balance, ["Cash And Cash Equivalents", "Cash and Cash Equivalents"], 4, 0))/ebitdaP1 if ebitdaP1 else 0
    ffo_pershareP1 = get_df_value(cashflow, ["Funds From Operations"], 4, 0) / sharesP1 if sharesP1 else 0
    price_to_ffoP1 = price_P1 /ffo_pershareP1 if ffo_pershareP1 else 0 
    repurchase_p1 = get_cashflow_value(
        cashflow,
        ["Repurchase of Stock", "Repurchase Of Stock", "Repurchases of Stock", "Repurchase of Common Stock"],
...
shares = get_df_value(balance, ["Share Issued", "Ordinary Shares Number", "Shares Outstanding"], 0, 0)
repo_pershare = get_cashflow_value(
    cashflow,
    ["Repurchase of Stock", "Repurchase Of Stock", "Repurchases of Stock", "Repurchase of Common Stock"],
    0,
    0,
) / shares if shares else 0
dividends_pershare = abs(get_df_value(cashflow, ["Cash Dividends Paid", "Dividends Paid"], 0, 0)) / shares if shares else 0
...
nav = (asset - liabilities) / shares if shares else 0
roe = income / equity if equity else 0
roa = income / asset if asset else 0
revenue_growth = (revenue / revenueP1) - 1 if revenueP1 else 0
earning_growth = (income / incomeP1) - 1 if incomeP1 else 0
eps_growth = (eps / epsP1) - 1 if epsP1 else 0
profit_margin = income / revenue if revenue else 0
operating_margin = get_df_value(financial, ["Operating Income", "Operating Income (Loss)"], 0, 0) / revenue if revenue else 0
payout_ratio = dividends / income if income else 0
price_to_book = mrkt_cap / equity if equity and mrkt_cap else 0
ev_ebitda = enterprise_value / ebitda if ebitda and enterprise_value else 0
interest_coverage = get_df_value(financial, ["Operating Income", "Operating Income (Loss)"], 0, 0) / get_df_value(financial, ["Interest Expense"], 0, 0) if get_df_value(financial, ["Interest Expense"], 0, 0) else 0
...
total_return = (dividends_pershare + repo_pershare) / price if price else 0
ffo_pershare = get_df_value(cashflow, ["Funds From Operations"], 0, 0) / shares if shares else 0
price_to_ffo = price / ffo_pershare if ffo_pershare else 0
...
dividend_yield = round(dividends/shares/price*100,2)
dividend_yieldP1 = round(dividendsP1/sharesP1/price_P1*100,2)
dividend_yield_show = str(dividend_yield) + "%"
dividend_yieldP1_show = str(dividend_yieldP1) + "%"
debt_equity = round(liabilities/equity,2)
debt_equityP1 = round(liabilitiesP1/equityP1,2)
roic = round(roic,2)
roicp1 = round((nopatP1/avg_investedcapitalP1),2)
nav = round(nav,2)
navP1 = round((assetP1-liabilitiesP1)/sharesP1,2)
roe = round(roe,2)
roa = round(roa,2)
roeP1 = round(roep1,2)
roaP1 = round(roaP1,2)
